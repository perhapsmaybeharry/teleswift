...
open class Update {
	...
	open var update_id: Int
	/// Optional. New incoming message of any kind — text, photo, sticker, etc.
	open var message: Message
	/// Optional. New version of a message that is known to the bot and was edited
	open var edited_message: Message
	/// Optional. New incoming inline query
//	open var inline_query: InlineQuery // NYI: Inline Queries are yet to be implemented.
	/// Optional. The result of an inline query that was chosen by a user and sent to their chat partner.
//	open var chosen_inline_result: ChosenInlineResult // NYI: Inline Queries are yet to be implemented.
	/// Optional. New incoming callback query
//	open var callback_query: CallbackQuery
	
	public init(_ fromJSON: JSON) throws {
...
//		callback_query = CallbackQuery()
	...
	public init(with_update_id: Int, with_message: Message = Message(), with_edited_message: Message = Message()) {
...
open class User: Equatable {
	...
	public init(with_id: Int, with_first_name: String, with_last_name: String = String(), with_username: String = String()) {
		id = with_id
...
open class Chat {
	...
	public init(with_id: String, with_type: String, with_title: String = String(), with_username: String = String(), with_first_name: String = String(), with_last_name: String = String()) {
		id = with_id
		type = with_type
		title = with_title
		username = with_username
...
open class Message {
	...
	open var forward_from: User
	/// Optional. For messages forwarded from a channel, information about the original channel
	open var forward_from_chat: Chat
	/// Optional. For forwarded messages, date the original message was sent in Unix time
	open var forward_date: Int
	/// Optional. For replies, the original message. Note that the Message object in this field will not contain further reply_to_message fields even if it itself is a reply.
	open var reply_to_message: Message?
	/// Optional. Date the message was last edited in Unix time
	open var edit_date: Int
	/// Optional. For text messages, the actual UTF-8 text of the message, 0-4096 characters.
	open var text: String
	/// Optional. For text messages, special entities like usernames, URLs, bot commands, etc. that appear in the text
	open var entities: [MessageEntity]
	/// Optional. Message is an audio file, information about the file
	open var audio: Audio
	/// Optional. Message is a general file, information about the file
	open var document: Document
	/// Optional. Message is a photo, available sizes of the photo
	open var photo: [PhotoSize]
	/// Optional. Message is a sticker, information about the sticker
	open var sticker: Sticker
	...
	open var venue: Venue
	/// Optional. A new member was added to the group, information about them (this member may be the bot itself)
	open var new_chat_member: User
	/// Optional. A member was removed from the group, information about them (this member may be the bot itself)
	open var left_chat_member: User
	/// Optional. A chat title was changed to this value
	open var new_chat_title: String
	/// Optional. A chat photo was changed to this value
	open var new_chat_photo: [PhotoSize]
	/// Optional. Service message: the chat photo was deleted
	open var delete_chat_photo: Bool
	/// Optional. Service message: the group has been created
	open var group_chat_created: Bool
	...
	open var supergroup_chat_created: Bool
	...
	open var channel_chat_created: Bool
	...
	open var migrate_to_chat_id: Int
	...
	open var migrate_from_chat_id: Int
	/// Optional. Specified message was pinned. Note that the Message object in this field will not contain further reply_to_message fields even if it is itself a reply.
	open var pinned_message: Message?
...
open class MessageEntity {
	...
	public init(with_type: String, with_offset: Int, with_length: Int, with_url: String = String(), with_user: User = User()) {
...
open class PhotoSize {
	...
	public init(with_file_id: String, with_width: Int, with_height: Int, with_file_size: Int = Int()) {
...
open class Audio {
	...
	public init(with_file_id: String, with_duration: Int, with_performer: String = String(), with_title: String = String(), with_mime_type: String = String(), with_file_size: Int = Int()) {
		file_id = with_file_id
...
open class Document {
	...
	public init(with_file_id: String, with_thumb: PhotoSize = PhotoSize(), with_file_name: String = String(), with_mime_type: String = String(), with_file_size: Int = Int()) {
		file_id = with_file_id
		thumb = with_thumb
...
open class Sticker {
	...
	public init(with_file_id: String, with_width: Int, with_height: Int, with_thumb: PhotoSize = PhotoSize(), with_emoji: String = String(), with_file_size: Int = Int()) {
		file_id = with_file_id
		width = with_width
		height = with_height
		thumb = with_thumb
...
open class Video {
	...
	public init(with_file_id: String, with_width: Int, with_height: Int, with_duration: Int, with_thumb: PhotoSize = PhotoSize(), with_mime_type: String = String(), with_file_size: Int = Int()) {
		file_id = with_file_id
		width = with_width
		height = with_height
		duration = with_duration
...
open class Voice {
	...
	public init(with_file_id: String, with_duration: Int, with_mime_type: String = String(), with_file_size: Int = Int()) {
		file_id = with_file_id
		duration = with_duration
		mime_type = with_mime_type
		file_size = with_file_size
...
open class Contact {
	...
	public init(with_phone_number: String, with_first_name: String, with_last_name: String = String(), with_user_id: Int = Int()) {
...
open class Location {
	...
	public init(with_longitude: Float, with_latitude: Float) {
...
open class Venue {
	...
	public init(with_location: Location, with_title: String, with_address: String, with_foursquare_id: String = String()) {
		location = with_location
		title = with_title
...
open class UserProfilePhotos {
	...
	open var total_count: Int
	/// Requested profile pictures (in up to 4 sizes each)
	open var photos: [[PhotoSize]]
	...
	public init(with_total_count: Int, with_photos: [[PhotoSize]]) {
...
open class File {
	...
	public init(with_file_id: String, with_file_size: Int = Int(), with_file_path: String = String()) {
		file_id = with_file_id
		file_size = with_file_size
...
//	open var request_contact: Bool
//	/// Optional. If True, the user's current location will be sent when the button is pressed. Available in private chats only
//	open var request_location: Bool
//	
//	public init(_ fromJSON: JSON) throws {
//		
//	}
...
//	open var inline_keyboard: [[InlineKeyboardButton]]
//	
//	public init(_ fromJSON: JSON) throws {
//		
//	}
...
//	open var callback_data: String
//	/// Optional. If set, pressing the button will prompt the user to select one of their chats, open that chat and insert the bot‘s username and the specified inline query in the input field. Can be empty, in which case just the bot’s username will be inserted.
//	open var switch_inline_query: String
//
//	public init(_ fromJSON: JSON) throws {
//		
//	}
...
//	open var inline_message_id: String
...
open class ChatMember {
	...
	public init(with_user: User, with_status: String) {
		user = with_user
		status = with_status
	}
}
